...
_QUESTION_PATTERNS = [
    ...
    # ── 定义 (续: 裸"是什么"兜底，放在因果/原因模式之后避免被其吞噬) ──
    (r"([^?？!！]+)是什么", QuestionType.WHAT_IS, QueryIntent.DEFINE),
    # "什么是X" 模式 (疑问词前置)
    (r"什么是([^?？!！]+)", QuestionType.WHAT_IS, QueryIntent.DEFINE),
...
_SVO_PATTERNS = [
    ...
    # "什么是X" → 定义查询 (疑问词前置，主体在后)
    (r'^什么是(.+)', 1, 'DEFINED_AS', 1),
    # "A是B的一种" → IS_A
...
# 单字功能词 — 永远不是概念候选
_FUNCTION_CHARS = {'的', '是', '和', '与', '了', '着', '过', '在', '有', '吗', '呢', '吧', '被', '把', '对', '从', '向', '由', '到', '给', '为', '以', '因'}
...
class SemParser:
    ...
    def _segment_and_tag(self, text: str) -> Tuple[List[str], List[str]]:
        """jieba 分词 + 词性标注"""
        # 始终用 pseg 分词 (它能正确拆分"龙是"→"龙"+"是"，而 jieba.cut 会合并)
    ...
    def _is_concept_candidate(self, token: str, pos: str) -> bool:
        ...
        if token in _NEGATION_MARKERS:
            return False
        if token in _FUNCTION_CHARS:
            return False  # 单字功能词: 的, 是, 在, ...
        # 名词、动词、形容词、专有名词、英文
        concept_pos = {'n', 'nr', 'ns', 'nt', 'nz', 'v', 'vn', 'a', 'an', 'eng'}
        if pos in concept_pos:
            # 单字专名/名词仍需谨慎：只放行中文内容词，拒绝标点/外文单字
            if len(token) >= 2:
                return True
            if re.match(r'^[\u4e00-\u9fff]$', token):
                return True  # 单字中文名词/动词: 龙, 人, 道, 气, ...
            return False
        # 或者在我们的领域词表中
        ...
        if re.match(r'^[\u4e00-\u9fff]{2,}$', token):
            return True
        # 非 POS 模式兜底：单字中文内容词（排除功能词）
        if not pos and re.match(r'^[\u4e00-\u9fff]$', token) and token not in _FUNCTION_CHARS:
            return True
        return False
    ...
    def _disambiguate(self, concepts: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for concept in concepts:
            if concept in subjects:
                ...
                # 模糊匹配: 单字/双字概念不扩展(避免"龙"→"一条龙服务", "原子"→"原子核")
                # 仅对3字以上的概念做模糊匹配
                if len(concept) <= 2:
                    resolved.append(concept)
                    continue
