...
- **语义臂** (每轮): 对比学习修复能量景观分离度，确认的关联 → 降低路径能量（通道加深）
- **序列臂** (每5轮): 从 POETIC_NEXT 提取的 14,800 对有向字对（如 春→风、不→知），非对称 Hebbian 注入建立方向性梯度
- **废退**: 长期未用 → 权重衰减（通道变浅）
...
| 有向字对 (序列臂) | 14,800 对 (POETIC_NEXT, conf>0.01) |
| 盲区检测因子 | 7 个独立因子 |
| 扫描速度 | ~2.7s/全因子全量扫描 |
| 学习速度 | ~25s/盲区 (搜索+提取+注入) |
| 序列臂训练速度 | ~16s/413 有向关联 |
...
│   │   ├── orchestrator.py          #     龙珠调度器 v3: 信号驱动闭环 + 序列臂
...
│   ├── auto_learn.py                #     7×24 自主学习守护进程 (旧版)
...
| 零 LLM 主唱 | ✅ | 自主学习引擎 + self_evolving能量推理；Ollama标记[DEPRECATED] |
| 能量景观是唯一知识源 | ✅ | from_landscape=True模式，盲区检测纯从景观拓扑 |
| 用进废退 | ✅ | 守护进程每轮 + 引擎每50查询自动衰减 |
| 有向序列学习 | ✅ | 序列臂每5轮注入14,800有向字对，非对称Hebbian建方向梯度 |
| 婴儿式成长 | ✅ | BabyCurriculum接入引擎+守护进程，自动阶段进阶 |
| 自主进化 | ✅ | 7×24守护进程 + 扫描→搜索→注入→验证→衰减闭环 |

> 2026-06-18: 新增序列臂，从 POETIC_NEXT 提取有向字对，每5轮训练方向性。首轮验证：413 个有向关联，额外耗时 ~16s。
